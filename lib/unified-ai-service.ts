...
/**
  ...
  private static getOpenAIClient(apiKey: string): OpenAI {
    ...
    if (!this.openaiClient || this.openaiApiKey !== apiKey) {
      this.openaiClient = new OpenAI({ apiKey, timeout: 30_000 })
  ...
  private static getGeminiClient(apiKey: string): GoogleGenerativeAI {
    ...
    if (!this.geminiClient || this.geminiApiKey !== apiKey) {
...
Return ONLY the JSON object. Be accurate and honest about your confidence level.`
  ...
  /**
   ...
   static async generateContent(params: GenerationParameters, options?: { provider?: AIProvider }): Promise<GeneratedContent[]> {
    ...
    const overrideText = await getCategoryOverride(params.category as any).catch(
      () => (CategoryPromptOverrides as any)[params.category] || ''
    )
    ...
    try {
      const selection = await this.getCurrentProvider(options?.provider, 'generate')
      cachedSettings = selection.settings
      ...
      const settings = cachedSettings ?? (
        await this.getCurrentProvider(undefined, 'generate').catch(() => ({ settings: undefined }))
      ).settings
      const fallbackEnabled = settings ? settings.aiEnableProviderFallback !== 'false' : true
      if (!fallbackEnabled) {
        return this.getFallbackContent(params)
      }

      if (options?.provider) {
        attemptedProviders.add(options.provider)
      }

      const preferenceOrder: AIProvider[] = []
      const pushUnique = (provider?: AIProvider) => {
        if (!provider) return
        if (!preferenceOrder.includes(provider)) {
          preferenceOrder.push(provider)
        }
      }
      ...
      for (const fallbackProvider of preferenceOrder) {
        ...
        const keyFromSettings = settings?.[`${fallbackProvider}ApiKey`] as string | undefined
        const envKey = process.env[`${fallbackProvider.toUpperCase()}_API_KEY`] || ''
        const apiKey = keyFromSettings && keyFromSettings.length >= 10 ? keyFromSettings : envKey
        if (!apiKey || apiKey.length < 10) continue

        const parsedMax = settings?.aiMaxTokens ? parseInt(settings.aiMaxTokens, 10) : undefined
        const parsedTemp = settings?.aiTemperature ? parseFloat(settings.aiTemperature) : undefined
        const maxTokensValue = Number.isFinite(parsedMax ?? Number.NaN) ? (parsedMax as number) : 2000
        ...
        const fallbackConfig: ProviderConfig = {
          apiKey,
          model:
            (settings?.[`${fallbackProvider}Model`] as string) ||
        ...
        try {
          ...
          switch (fallbackProvider) {
            case 'openai':
              result = await this.callOpenAI(fallbackConfig, prompt)
              break
            case 'gemini':
              result = await this.callGemini(fallbackConfig, prompt)
              break
            case 'deepseek':
              result = await this.callDeepSeekForGeneration(fallbackConfig, prompt)
              break
          }
          const parsed = this.parseGenerationResponse(result, params)
          console.log(
            `[UnifiedAI] Fallback provider ${fallbackProvider.toUpperCase()} succeeded with ${parsed.length} items`
          )
          return parsed
        } catch (fallbackError) {
          console.warn(
            `[UnifiedAI] Fallback provider ${fallbackProvider.toUpperCase()} failed:`,
  ...
  private static parseGenerationResponse(response: string, params: GenerationParameters): GeneratedContent[] {
    try {
      ...
      const minLength = params.type === 'quote' ? 5 : 10
      ...
      return items
        ...
        .filter((item): item is GeneratedContent => !!item && item.content.trim().length >= minLength)
