...
(function main() {
  try {
    ...
    const db = process.env.POSTGRES_URL ? String(process.env.POSTGRES_URL).trim() : '';
    if (!db) {
      console.error('[ensure-env] Missing DIRECT_URL and POSTGRES_URL. Prisma schema expects DIRECT_URL.');
      console.error('[ensure-env] Set DIRECT_URL in Vercel (recommended: same as POSTGRES_URL) or provide both for pooled/non-pooled setups.');
      process.exit(1);
    }
    ...
    if (!hadDirectInFile) {
      ...
      console.log('[ensure-env] Synthesized DIRECT_URL into .env from POSTGRES_URL for build-time Prisma get-config.');
    } else {
